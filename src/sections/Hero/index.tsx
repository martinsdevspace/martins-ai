...
export default async function Hero() {
  ...
  const metrics =
    siteSettings.metrics?.filter((m) => m.value && m.label) || []

  const terminalLines =
    siteSettings.terminalLines
      ?.map((line) => [line.prompt || '$', line.text || ''] as [string, string])
      .filter(([, text]) => Boolean(text)) || []

  const primaryLabel = siteSettings.cta?.primaryLabel
  const primaryTo = siteSettings.cta?.primaryTo
  const secondaryLabel = siteSettings.cta?.secondaryLabel
  const secondaryTo = siteSettings.cta?.secondaryTo
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[1fr_1.1fr] lg:gap-12 items-center">
        <div className="flex flex-col items-start">
          ...
          {siteSettings.heroIntro && typeof siteSettings.heroIntro === 'object' ? (
        ...
        {terminalLines.length > 0 ? (
          <aside className="hidden lg:flex flex-col border border-border bg-carbon text-background">
            ...
            <div className="p-5 font-mono text-xs leading-relaxed space-y-2">
              {terminalLines.map(([prompt, text], i) => (
